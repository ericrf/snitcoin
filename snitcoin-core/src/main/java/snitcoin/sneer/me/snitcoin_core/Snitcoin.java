...
public class Snitcoin implements Runnable {
	...
	public void run() {
        ...
        for (Transaction t : ts) {
        	...
        	transactions.add(new snitcoin.sneer.me.snitcoin_core.Transaction(t.getHashAsString(), t.getValue(kit.wallet()).toPlainString(), "" + progress));
		}
	...
	private class WalletEventListenerImpl extends AbstractWalletEventListener implements WalletEventListener {
		...
		private void addTransaction(Transaction tx) {
			...
			transactions.add(new snitcoin.sneer.me.snitcoin_core.Transaction(hash, amount, "0.0"));
		}
				...
				public void onBroadcastProgress(double progress) {
					for (int i = 0; i < transactions.size(); i++) {
						...
						if (t.hash.equals(tx.getHashAsString())) {
							transactions.set(i, new snitcoin.sneer.me.snitcoin_core.Transaction(t.hash, t.amount, String.valueOf(progress)));
